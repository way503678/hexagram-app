...
## 0m. 首頁問候間距與滑動調整（2026-09-14）

- 移除「命果」與問候之間的大面積留白；一般手機間距縮為 16px，窄螢幕或放大字體時縮為 8px。
- 固定滿版背景與「命果」標題維持不動，問候文字改與今日指引、黃曆及按鈕一起滑動。
- 實機尺寸預覽、滑動行為、`tsc --noEmit` 與 Expo Doctor 18/18 驗證通過。
- 雙平台內部測試發布完成：iOS build 11 已提交 TestFlight，Android versionCode 9 已提交 Google Play internal；App 內容來源 commit `49a02e0`。

## 0l. 首頁固定滿版背景（2026-09-08）

- 首頁標題移除英文 `MINGO`，只保留中文「命果」。
- 山景圖改為獨立的 100% 寬高固定背景層；品牌與問候固定，只有今日指引、黃曆與操作按鈕所在的前景卡片區可滑動。
- 手機尺寸預覽與滑動行為驗證通過；`tsc --noEmit`、Expo Doctor 18/18 通過。
- 雙平台內部測試發布完成：iOS build 10 已提交 TestFlight，Android versionCode 8 已提交 Google Play internal；App 內容來源 commit `d104e30`。

## 0k. 卜卦爻象與六爻詳表對齊（2026-09-05）

- 擲卦區與排盤結果不再用 Unicode 橫線字串描繪爻象，改用固定寬度的原生線段元件，陰爻缺口及動爻叉／圈固定置中，避免 iOS、Android 字型寬度差造成六列錯位。
- 六爻本卦詳表移除「上爻／五爻……」爻序欄，改為爻象、六神、六親、干支四欄；世爻／應爻放在爻象下方。App 不再顯示旺衰／比和資訊，也不在動爻干支後加「動」字；動爻仍以紅色爻象與變出資訊辨識。
...
- 2026-09-05 雙平台測試版發布完成：iOS build 7 已在 TestFlight 內部測試，Android versionCode 5 已送達 Google Play internal；來源 commit `5bc55af`。

## 0h. 萬年曆七欄與日期詳情修正（2026-09-04）

- 月曆由百分比寬度自動換行改為明確的七欄逐列配置，星期標題與日期共用相同欄位分配，避免 iOS 像素取整或放大字級時星期六換到下一列／消失。
- 依後端 `first_weekday`（週一為 0）轉成畫面週日起始欄位，並核對 2026 年各月份的星期六日期位置。
...
## 0j. 啟動、首頁導向、生日選擇與 Home 圖示（2026-09-04）

- 原生 Splash 移除圖片，只保留與 App 載入畫面一致的暖米底色，避免啟動瞬間閃過 Splash 圖。
- 主分頁明確指定 `Home` 為初始頁，登入或還原登入狀態後直接進首頁。
- 個人生日改為年、月、日、時四欄獨立選擇；年份可從 1900 年選到今年，月份與日期會依閏年、當月天數及今天自動限制。
- 中央 Home 分頁按鈕由品牌 mark 改為房子圖示。

## 0g. 品牌圖片與圖示更新（2026-09-02）

- 依 `/opt/icon` 新版品牌稿更新 App icon、中文 Logo、品牌 mark、favicon、Android adaptive icon 與 native splash。
- 將 15 個新版深靛藍線條圖示整理為無文字透明 PNG；介面標籤仍由 React Native 原生文字顯示，避免縮圖時重複或模糊。
- 現有三分頁架構不變；左側「功能」視覺名稱改為「探索」，黃曆與卜卦改用新版「指引／占卜」圖示。
- 依品牌稿產生無字的暖米紫山景背景與晨光山谷首頁主圖，取代舊版幾何佔位圖。
- Web 共用 Logo、mark 與 favicon 同步更新於 `/opt/hexagram/static/`。
- 修正 TestFlight 實機的卜卦頁跑版：iOS 返回鍵不再顯示內部路由名 `Tabs`；排盤後摘要改成可換行的上下配置；390pt 以下寬度或放大字級時縮小頁面／卡片留白，六爻與變卦欄位可自動縮字而不互相擠壓；空卦名資料不再留下白色空卡。

## 0f. 萬年曆選日日期改「年月日」(2026-07-02)

- `AlmanacScreen` 選日詳情標題 `sel.solar`(2026-07-05)→ 年月日格式(2026年7月5日),對齊 web 萬年曆彈窗。純顯示格式,tsc 通過。
